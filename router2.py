...
# ---------- New Endpoints: PROMPTS and FREQUENT QUESTIONS ----------
...
# GET prompts for a given application code
@route.get("/prompts")
def get_prompts(aplctn_cd: str):
    file_name = f"{aplctn_cd}_prompts.json"
    if not Path(file_name).exists():
        raise HTTPException(status_code=404, detail="Prompt file not found.")

    with open(file_name, "r") as f:
        data = json.load(f)
        return {"application": aplctn_cd, "prompts": data.get(aplctn_cd, [])}

# POST to add a new prompt
@route.post("/prompts")
def add_prompt(prompt: PromptModel):
    file_name = f"{prompt.aplctn_cd}_prompts.json"
    ...
    data.setdefault(prompt.aplctn_cd, [])

    if any(p["prompt_name"] == prompt.prompt_name for p in data[prompt.aplctn_cd]):
    ...
    data[prompt.aplctn_cd].append(new_prompt)

    with open(file_name, "w") as f:
        json.dump(data, f, indent=2)

    return {"message": "Prompt added", "prompt": new_prompt}

# GET frequent questions for a given application and user context
@route.get("/frequent_questions")
def get_frequent_questions(aplctn_cd: str, user_context: str):
    file_name = f"{aplctn_cd}_freq_questions.json"
    if not Path(file_name).exists():
        raise HTTPException(status_code=404, detail="Frequent question file not found.")

    with open(file_name, "r") as f:
        data = json.load(f)
        questions = [
            q for q in data.get(aplctn_cd, [])
            if q["user_context"] == user_context
        ]
        return {"application": aplctn_cd, "user_context": user_context, "questions": questions}

# POST to add a frequent question
@route.post("/frequent_questions")
def add_frequent_question(q: FrequentQuestionModel):
    file_name = f"{q.aplctn_cd}_freq_questions.json"
    if Path(file_name).exists():
        with open(file_name, "r") as f:
            data = json.load(f)
    else:
    ...
    data.setdefault(q.aplctn_cd, [])

    existing_prompts = {
        d["prompt"] for d in data[q.aplctn_cd] if d["user_context"] == q.user_context
    }

    if q.prompt in existing_prompts:
        raise HTTPException(status_code=409, detail="Question already exists.")

    new_question = {"user_context": q.user_context, "prompt": q.prompt}
    data[q.aplctn_cd].append(new_question)

    with open(file_name, "w") as f:
        json.dump(data, f, indent=2)

    return {"message": "Frequent question added", "question": new_question}
