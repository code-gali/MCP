...
@mcp.resource("genaiplatform://{aplctn_cd}/prompts/{name}/{description}")
async def prompt_library(aplctn_cd: str, name: str, description: str) -> List[dict]:
    resource_name = aplctn_cd + "_prompts.json"
    prompt_data = json.load(open(resource_name))
    aplcn_prompts = prompt_data.get(aplctn_cd)
    return [{"name": rec["name"], "description": rec["description"], "messages": rec["messages"]} 
            for rec in aplcn_prompts if rec["name"] == name and rec["description"] == description]
@mcp.tool(
        name="add-prompts",
        description=""" 
        ...
        Args:
            ...
            prompts (list): List of prompt definitions with name, description and messages
        """
)
async def add_prompts(ctx: Context, uri: str, prompts: list) -> list:
    ...
    return [{"name": prompt["name"], "description": prompt["description"], "messages": prompt["messages"]} 
            for prompt in file_data[aplctn_cd]]
