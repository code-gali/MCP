...
# Test connection on startup and add retry button
                ...
                try:
                    async with sse_client(url=server_url) as sse:
                        async with ClientSession(*sse) as session:
                            ...
                            # First verify the prompt data
                            if not prompt_name or not prompt_desc or not prompt_content:
                                st.error("❌ All prompt fields must be filled out")
                                return
                            ...
                            try:
                                ...
                                # Check if the prompt was actually added by verifying the file
                                if os.path.exists("hedis_prompts.json"):
                                    with open("hedis_prompts.json", 'r') as f:
                                        prompt_data = json.load(f)
                                        if "hedis" in prompt_data:
                                            for p in prompt_data["hedis"]:
                                                if p["prompt_name"] == prompt_name:
                                                    ...
                                                    st.json(result.model_dump() if result else prompt_data)
                                                    # Reload available items after adding
                                                    st.session_state.available_items = load_available_items()
                                                    st.rerun()
                                                    return
                                    
                                st.error("❌ Failed to verify prompt addition")
                                    
                            except Exception as tool_error:
                                # Check if the prompt was actually added despite the error
                                if os.path.exists("hedis_prompts.json"):
                                    with open("hedis_prompts.json", 'r') as f:
                                        prompt_data = json.load(f)
                                        if "hedis" in prompt_data:
                    ...
                    error_msg = str(e)
                    # Check if the prompt was actually added despite the TaskGroup error
                    if "TaskGroup" in error_msg and os.path.exists("hedis_prompts.json"):
                        with open("hedis_prompts.json", 'r') as f:
                            prompt_data = json.load(f)
                            if "hedis" in prompt_data:
                                for p in prompt_data["hedis"]:
                    ...
                    st.error(f"❌ Failed to add prompt: {error_msg}")
